...
if ($conn->connect_error) {
  ...
  $sql="select `kd_nilai`,`kd_pendaftaran`,`kd_periode`,`kd_kursus`,`kd_kursus_dibuka`,`nrp`, `keterangan` from `$tb_nilai` order by `kd_nilai` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
	  $arr=getData($conn,$sql);
	  foreach($arr as $d) {		 
					$value=$d["kd_nilai"];$buffer .= "\"".$value."\"".$separator; 
