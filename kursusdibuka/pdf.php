...
class PDF extends FPDF{
  function Header(){
    ...
    $this->Cell(20,0,'Data Kursus Dibuka',0,0,'L');
    $this->Ln();
    $this->Cell(5,1,'Laporan data Kursus Dibuka',0,0,'L');
    $this->Ln();
...
$arr=getData($conn,$sql);
  ...
  $cell[$i][2]=$d["nama_subkursus"];
  $cell[$i][3]=getPeriode($conn, $d["kd_periode"]);
  $cell[$i][5]=$d["tingkat"];
   
	    $cell[$i][11]=WKT($d["ujian_masuk"]);
		 $cell[$i][12]=WKT($d["buka"]);
		  $cell[$i][13]=WKT($d["ujian_tengah"]);
		   $cell[$i][14]=WKT($d["ujian_akhir"]);
		    $cell[$i][15]=WKT($d["tutup"]);
...
$pdf->Cell(6,1,'nama_subkursus','LR',0,'C',1);
$pdf->Cell(3,1,'kd_periode','LR',0,'C',1);

$pdf->Cell(2,1,'tingkat','LR',0,'C',1);


$pdf->Cell(3,1,'ujian_masuk','LR',0,'C',1);
$pdf->Cell(3,1,'buka','LR',0,'C',1);
$pdf->Cell(3,1,'ujian_tengah','LR',0,'C',1);
$pdf->Cell(3,1,'ujian_akhir','LR',0,'C',1);
$pdf->Cell(3,1,'tutup','LR',0,'C',1);
$pdf->Ln();
$pdf->SetFont('Arial','','8');
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(6,1,$cell[$j][2],'B',0,'L'); // nama_subkursus
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // kd_periode
  
  $pdf->Cell(2,1,$cell[$j][5],'B',0,'L'); // tingkat
  
   
	    $pdf->Cell(3,1,$cell[$j][11],'B',0,'L'); // ujian_masuk
		 $pdf->Cell(3,1,$cell[$j][12],'B',0,'L'); // buka
		  $pdf->Cell(3,1,$cell[$j][13],'B',0,'L'); // ujian_tengah
		   $pdf->Cell(3,1,$cell[$j][14],'B',0,'L'); // ujian_akhir
			$pdf->Cell(3,1,$cell[$j][15],'B',0,'L'); // tutup
