...
class PDF extends FPDF{
  function Header(){
    ...
    $this->Cell(20,0,'Data surat',0,0,'L');
    $this->Ln();
    $this->Cell(5,1,'Laporan data surat',0,0,'L');
    $this->Ln();
...
$arr=getData($conn,$sql);
  ...
  $cell[$i][5]=$d["nrp"];
  $cell[$i][6]=$d["status"];
  $cell[$i][7]=$d["nomor_surat"];
  $cell[$i][8]= WKT($d["tanggal_pembuatan"]);
  $i++;
}
...
$pdf->Cell(3,1,'Kode Surat','LR',0,'C',1);
...
$pdf->Cell(2,1,'Nomor Surat','LR',0,'C',1);
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(3,1,$cell[$j][0],'B',0,'L'); // kd_surat
  ...
  $pdf->Cell(2,1,$cell[$j][6],'B',0,'L'); // status
   $pdf->Cell(2,1,$cell[$j][7],'B',0,'L'); // nomor_surat
  $pdf->Cell(3,1,$cell[$j][8],'B',0,'L'); // tanggal_pembuatan
  $pdf->Ln();
}
$pdf->Output();
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
