...
if ($conn->connect_error) {
  trigger_error('Database connection failed: '  . $conn->connect_error, E_USER_ERROR);
	...
	"nrp".$separator; 
	"status".$separator; 
	"nomor_surat".$separator;
    $buffer .= $newline; 
    
  $sql="select `kd_surat`,`kd_pendaftaran`,`kd_periode`,`kd_kursus`,`kd_kursus_dibuka`,`nrp`, `status`, `nomor_surat` from `$tb_surat` order by `kd_surat` desc";
  $jum=getJum($conn,$sql);	
  if($jum>0){						
	  $arr=getData($conn,$sql);
	  foreach($arr as $d) {		 
					$value=$d["kd_surat"];$buffer .= "\"".$value."\"".$separator; 
					...
					$value=$d["nomor_surat"];$buffer .= "\"".$value."\"".$separator; 
				$buffer .= $newline; 
		}	
  }
  else{
